...
from swap_test_circuit import compute_similarity, qubit_by_qubit_swaptest, run_circuit # For SWAP test and circuit execution
import os
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        st.markdown("Use the buttons below to move the blue dot (Test Location) on the map. Adjust the step size for finer or coarser movement.")

        # Slider to control the step size for button-based navigation.
        step_size = st.slider(label="Movement Step Size (pixels)", min_value=5, max_value=50, value=10, step=5,
        ...
        A_ref_val = st.slider(label="Reference RSSI (A_ref, dBm)", min_value=-60.0, max_value=-20.0, value=-40.0, step=1.0,
                              help="RSSI value measured at the reference distance d0 (typically 1m).")
        n_val_ple = st.slider(label="Path Loss Exponent (n)", min_value=1.5, max_value=6.0, value=3.0, step=0.1,
                              help="Rate at which signal strength decreases with distance. Higher is faster attenuation.")
        d0_val = st.slider(label="Reference Distance (d0, m)", min_value=0.1, max_value=5.0, value=1.0, step=0.1,
        ...
        noise_std_dev = st.slider(label="RSSI Noise Std Dev (dBm)", min_value=0.0, max_value=10.0, value=0.0, step=0.1,
        ...
        num_samples_for_avg = st.slider(label="RSSI Samples for Averaging", min_value=1, max_value=20, value=1,
        ...
        fp_rssi_database_computed = compute_rssi_d(fp_database, ap_database, 
        ...
        blue_dot_scatter = ax.scatter(x=[st.session_state.x_p], y=[st.session_state.y_p], c='b', s=40, label="Test Location")


        # Compute RSSI vector for the current test location using session state coordinates and simulation settings.
        rssi_test_vector = compute_rssi([st.session_state.x_p, st.session_state.y_p], ap_database, 
                                        noise_std_dev=noise_std_dev, 
                                        num_samples_for_avg=num_samples_for_avg,
                                        A_ref=A_ref_val,
                                        n_val=n_val_ple,
        ...
        if fp_rssi_probability_database:
            similarity_data = [{"Reference Point": fp_l_database[key], "Similarity Score": score} 
    ...
    with st.expander("Quantum Circuit Implementing the Fingerprint Matching"):
        ...
        if fp_rssi_database_computed and prob_loc_pass in fp_rssi_database_computed:
            ...
            st.markdown("Fingerprint data not yet computed or probable location is invalid. Displaying with potentially stale/default data.")
        ...
        if display_vector_dim > 0:
            ...
            if display_vector_dim < display_required_dim:
                display_v1_padded = np.pad(display_v1_state, (0, display_required_dim - display_vector_dim), 'constant', constant_values=0.0)
                display_v2_padded = np.pad(display_v2_state, (0, display_required_dim - display_vector_dim), 'constant', constant_values=0.0)
            else:
            ...
            if display_norm_v1 == 0 or display_norm_v2 == 0:
                st.markdown("One of the vectors for display (after padding) is zero, cannot generate or display quantum circuit.")
        ...
        if display_vector_dim > 0: 
            ...
            st.markdown(f"   - Fingerprint Vector ({fp_l_database[prob_loc_pass if prob_loc_pass else list(fp_l_database.keys())[0]]}): `{np.round(display_v2_state, 3)}`")
            ...
            if len(display_v1_state) != len(display_v1_padded): 
                ...
                st.markdown(f"**3. Normalized Padded Vectors (for Quantum State Encoding - {num_encoding_qubits} qubits)**")
            else: 
                st.markdown(f"**2. Normalized Original Vectors (for Quantum State Encoding - {num_encoding_qubits} qubits)**")
            ...
            if display_norm_v1 != 0 and display_norm_v2 != 0:
                ...
                st.markdown("   - One or both vectors are zero (or became zero after padding), so normalization is not applicable or results in a zero vector. Circuit cannot be run.")
        else:
        ...
        if qc and display_vector_dim > 0 and (display_norm_v1 != 0 and display_norm_v2 != 0) : 
            ...
            st.markdown("Quantum circuit and histogram cannot be displayed, likely because one or both input vectors were zero or empty.")
